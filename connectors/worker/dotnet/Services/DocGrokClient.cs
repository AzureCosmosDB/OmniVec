...
public class DocGrokClient
{
    ...
    public DocGrokClient(
        HttpClient http, ILogger<DocGrokClient> logger, MetricsReporter? metrics = null)
    {
        _http = http;
        _logger = logger;
        _metrics = metrics;
    ...
    private async Task<JsonDocument> PostAsync(
        string path, Dictionary<string, object?> request, CancellationToken ct,
        string modelId = "", string pipelineId = "", string sourceId = "",
        string destinationId = "", string sourceRef = "", bool inline = false)
    {
        for (var attempt = 1; ; attempt++)
        {
            ...
            try
            {
                ...
                {
                    await ReportModelAttemptAsync(
                        pipelineId, modelId, sourceId, destinationId, sourceRef,
                        retryCount: 0, throttleCount: status == 429 ? 1 : 0,
                        throttleDelayMs: 0, errorCategory: $"http_{status}", ct: ct);
                ...
                await ReportModelAttemptAsync(
                    pipelineId, modelId, sourceId, destinationId, sourceRef,
                    retryCount: 1, throttleCount: status == 429 ? 1 : 0,
                    throttleDelayMs: status == 429 ? delay.TotalMilliseconds : 0,
                    errorCategory: $"http_{status}", ct: ct);
            }
            catch (HttpRequestException) when (attempt < MaxAttempts)
            {
                ...
                await ReportModelAttemptAsync(
                    pipelineId, modelId, sourceId, destinationId, sourceRef,
                    retryCount: 1, errorCategory: "network", ct: ct);
            }
            catch (OperationCanceledException) when (!ct.IsCancellationRequested && attempt < MaxAttempts)
            {
                ...
                await ReportModelAttemptAsync(
                    pipelineId, modelId, sourceId, destinationId, sourceRef,
                    retryCount: 1, errorCategory: "timeout", ct: ct);
            }
            catch (OperationCanceledException ex) when (!ct.IsCancellationRequested)
            {
                await ReportModelAttemptAsync(
                    pipelineId, modelId, sourceId, destinationId, sourceRef,
                    retryCount: 0, errorCategory: "timeout", ct: ct);
    ...
    private async Task ReportModelAttemptAsync(
        string pipelineId, string modelId, string sourceId, string destinationId,
        string sourceRef, int retryCount, string errorCategory,
        CancellationToken ct, int throttleCount = 0, double throttleDelayMs = 0)
    {
        if (_metrics is null || string.IsNullOrWhiteSpace(pipelineId)) return;
        await _metrics.ReportInlineMetricsAsync(
            pipelineId, 0, 0, 0,
            $"model-attempt:{Guid.NewGuid():N}", ct,
            sourceId: sourceId, destinationId: destinationId, modelId: modelId,
            retryCount: retryCount, throttleCount: throttleCount,
            throttleDelayMs: throttleDelayMs, errorCategory: errorCategory,
            lastDocument: sourceRef);
    }
    ...
    public async Task<List<float[]>> EmbedBatchAsync(
        ...
        string pipelineId = "", string sourceId = "", string destinationId = "",
        string sourceRef = "")
    {
        var request = RequestFor(modelOrPipeline);
        request["texts"] = texts;
        using var json = await PostAsync(
            "/embed/batch", request, ct, modelOrPipeline, pipelineId,
            sourceId, destinationId, sourceRef);
        ...
        string pipelineId = "", string sourceId = "", string destinationId = "")
    {
        ...
        using var json = await PostAsync(
            "/embed", request, ct, modelOrPipeline, pipelineId,
            sourceId, destinationId, blobName);
        ...
        string pipelineId = "", string sourceId = "", string destinationId = "")
    {
        ...
        using var json = await PostAsync(
            "/process", request, ct, modelOrPipeline, pipelineId,
            sourceId, destinationId, fileName, inline: true);
        ...
        string pipelineId = "", string sourceId = "", string destinationId = "")
    {
        ...
        using var json = await PostAsync(
            "/embed", request, ct, modelOrPipeline, pipelineId,
            sourceId, destinationId, blobNames[^1]);
