...
```bash
curl -X POST http://<external-ip>/api/sources \
...
```bash
# Test before saving
curl -X POST http://<external-ip>/api/sources/test-connection \
  -H "Content-Type: application/json" \
  -d '{
    "type": "azure-blob",
    "config": {
...
curl -X POST http://<external-ip>/api/sources/src-abc12345/test
```
...
```bash
curl -X POST http://<external-ip>/api/destinations \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST http://<external-ip>/api/destinations/test-connection \
  -H "Content-Type: application/json" \
  -d '{
    "type": "cosmosdb-vector",
    "config": {
...
```bash
curl http://<external-ip>/api/docgrok/pipelines
```

### Create a Custom Pipeline

```bash
curl -X POST http://<external-ip>/api/docgrok/pipelines \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST http://<external-ip>/api/pipelines \
  -H "Content-Type: application/json" \
  -d '{
...
curl http://<external-ip>/api/pipelines

# Get specific pipeline
curl http://<external-ip>/api/pipelines/pip-abc12345

# Pause a pipeline
curl -X POST http://<external-ip>/api/pipelines/pip-abc12345/pause

# Resume a pipeline
curl -X POST http://<external-ip>/api/pipelines/pip-abc12345/resume

# Activate for processing
curl -X POST http://<external-ip>/api/pipelines/pip-abc12345/run

# Delete a pipeline
curl -X DELETE http://<external-ip>/api/pipelines/pip-abc12345
```
...
curl http://<external-ip>/api/jobs

# Filter by pipeline
curl "http://<external-ip>/api/jobs?pipeline_id=pip-abc12345"

# Filter by status
curl "http://<external-ip>/api/jobs?status=failed"

# Get job stats
curl http://<external-ip>/api/jobs/stats

# Retry a failed job
curl -X POST http://<external-ip>/api/jobs/job-abc12345678/retry

# Cancel a pending job
curl -X POST http://<external-ip>/api/jobs/job-abc12345678/cancel
```
...
curl -X POST http://<external-ip>/api/triggers/eventgrid/create \
  -H "Content-Type: application/json" \
  -d '{
    "source_id": "src-abc12345"
  }'
...
curl http://<external-ip>/api/triggers/eventgrid/list

# Check trigger status
curl http://<external-ip>/api/triggers/status

# Delete a trigger
curl -X DELETE http://<external-ip>/api/triggers/eventgrid/src-abc12345
```
...
curl http://<external-ip>/api/operations/deployments
```
...
[
  {
    ...
    "status": "Running",
    "image": "<internal-acr>.azurecr.io/omnivec-api:v50",
    "pods": [
...
curl -X POST http://<external-ip>/api/operations/deployments/omnivec-worker/scale \
  -H "Content-Type: application/json" \
  -d '{"replicas": 3}'

# Pause controller (scale to 0)
curl -X POST http://<external-ip>/api/operations/deployments/omnivec-controller/scale \
  -H "Content-Type: application/json" \
  -d '{"replicas": 0}'
...
curl -X POST http://<external-ip>/api/operations/deployments/omnivec-worker/restart
```
...
```bash
curl http://<external-ip>/api/metrics
```
...
```bash
curl -X POST http://<external-ip>/api/playground/search \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl "http://<external-ip>/api/jobs?pipeline_id=pip-XXXXXXXX"
```
...
az acr build --registry <internal-acr> --image omnivec-api:vXX --file api/Dockerfile .

# Deploy to all components
kubectl set image deployment/omnivec-api api=<internal-acr>.azurecr.io/omnivec-api:vXX -n omnivec
kubectl set image deployment/omnivec-controller controller=<internal-acr>.azurecr.io/omnivec-api:vXX -n omnivec
kubectl set image deployment/omnivec-worker worker=<internal-acr>.azurecr.io/omnivec-api:vXX -n omnivec
```
...
| ACR | `<internal-acr>` | Image: `omnivec-api` |
...
| External IP | `<external-ip>` | Load balancer for API |
