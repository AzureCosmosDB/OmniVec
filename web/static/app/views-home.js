...
route('home', { needs:[...window.OVX.CORE, 'series'], render() {
  ...
  const indexed = ps.reduce((sum, p) => sum + Number(p.st.embedded_count || 0), 0);
  const recordedFailures = ps.reduce((sum, p) => sum + Number(p.failed || 0), 0);
  const html = `<div class="page">
   ...
   <div class="card stats" style="margin-top:16px">
    <div class="stat"><div class="lbl">Vectors indexed</div><div class="val">${ps.some(p => p.statsLoading) ? '<div class="skel" style="height:26px;width:90px"></div>' : fmt(indexed)}</div><div class="row sp"><span class="sub">Verified from destination stores</span>${spark(hist)}</div></div>
    <div class="stat"><div class="lbl">Failed · recorded</div><div class="val" style="color:${recordedFailures ? 'var(--err)' : 'var(--ok)'}">${fmt(recordedFailures)}</div><div class="sub">Durable pipeline status across all replicas</div></div>
    <div class="stat"><div class="lbl">Embedding latency · API pod</div>${embLat.count ? `<div class="val">${ms(embLat.avg)}<small>avg</small></div><div class="sub">p95 ${ms(embLat.p95)} · ${fmt(embLat.count)} local samples</div>` : `<div class="val na" style="font-size:16px;margin-top:8px">Not recorded</div><div class="sub">No local latency samples on this API pod</div>`}</div>
    <div class="stat"><div class="lbl">Workers</div>${W ? `<div class="val">${W.ready_replicas??'—'}<small>/ ${W.replicas} ready</small></div><div class="row sp"><span class="sub">${W.autoscaling ? 'Autoscale '+W.autoscaling.min_replicas+'–'+W.autoscaling.max_replicas : 'Fixed replicas'} · Service Bus queue available in operations</span><span class="pod">${Array.from({length:Math.min(10,(W.autoscaling&&W.autoscaling.max_replicas)||W.replicas||0)},(_,i)=>`<i class="${i < (W.ready_replicas||0) ? '' : 'off'}"></i>`).join('')}</span></div>` : '<div class="val na" style="font-size:16px;margin-top:8px">Not observed</div>'}</div>
   </div>
   <div class="grid g-main" style="margin-top:16px;align-items:start">
    ...
    <div class="stack">
      <div class="card"><div class="card-h"><h3>Telemetry samples · 24h</h3>${S && S.coverage && S.coverage !== 'hourly' ? `<span class="badge warn" title="Some pipelines only have their last 60 worker reports, so this chart undercounts. Full hourly history builds up from new activity.">Partial history</span>` : ''}<div class="acts legend"><span><i style="background:var(--accent)"></i>reported</span><span><i style="background:var(--warn)"></i>avg latency</span></div></div>
        <div class="card-b">${buckets.length ? chart(buckets.map(b => (b.t||'').slice(11,16)), [{vals:hist,color:'var(--accent)',type:'bar'},{vals:buckets.map(b=>b.avg_latency_ms||0),color:'var(--warn)',type:'line',axis:'r',fmt:ms}], 170) : empty('chart','No telemetry samples in the last 24 hours','Exact indexed totals are shown above.')}</div></div>
